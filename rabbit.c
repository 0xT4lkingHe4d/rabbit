/* Rabbit Stream Cipher Implementaion
...
void set_up_key_vars(rabbit_t *cr, unsigned char *key) {
	int pos = 0;
...
int init_variables(rabbit_t *cr) {
...
void next_state_func(rabbit_t *cr) {
	counter_system(cr);
...
void counter_carry_bit(rabbit_t * cr) {
	// puts("[+++] NEW ROUND [+++]");
	uint8_t tmp;
	for (int j = 0; j < 8; j++)
...
void counter_system(rabbit_t * cr) {
	counter_carry_bit(cr);
	for (int j = 0; j < 8; j++) {
		cr->C[j] = (cr->C[j]+A_const[j] + cr->Carry[j+1]) % MOD_VAL;
	}
}

void extraction_scheme(rabbit_t * cr) {
...
void rabbit_do_ecrypt(rabbit_t *cr, unsigned char *msg, unsigned char *out) {
	for (int i = 0; i < 64; i++) {
...
int rabbit_key_stream(unsigned char *key, rabbit_t * cr) {
	...
	set_up_key_vars(cr, key);
	int err = init_variables(cr);
	if (err != 0)
		return -1;		// NEW_ERROR
...
// 	char *in = malloc(64), *out = malloc(64);
// 	char *str = "AAAAABBBBBCCCCCDDDDDEEEEEFFFFFAAAAABBBBBCCCCCDDDDDEEEEEFFFFF";
// 	rabbit_do_ecrypt(cr, str, out);
// 	rabbit_do_ecrypt(cr, out, in);
// 	printf("%s\n", in);
